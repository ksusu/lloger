...
// Arguments tests
module.exports.check_Command = function check_Command(COMMAND, COM_ARGS){
...
module.exports.classify_Command = function classify_Command (hosturl ,COMMAND, COM_ARGS) {
	...
	if (COMMAND == 'register') {
		...
		var action = ""
		for( i=0; i < COM_ARGS.length; i++) {
			action = action + COM_ARGS[i]
			if(i =! COM_ARGS.lenth - 1) {
				action = action + '#';
			}
		}
		if (action == '') {
	...
	const options = {
		...
		method: 'POST',
		headers: {
		'authorization': "Basic " + new Buffer(userID + ":" + userPW, "utf-8").toString('base64')
		}
	}
	return options;	
}